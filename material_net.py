...
def srgb_to_linear(inputs):
    # Precise sRGB to linear conversion (piecewise)
    mask = inputs <= 0.04045
    linear_low = inputs / 12.92
    linear_high = ((inputs + 0.055) / 1.055) ** 2.4
    return tf.where(mask, linear_low, linear_high)
...
def _parse_function(filename):
    ...
    # Apply sRGB to linear conversion to inputs (replaces approximate gamma correction)
    if a.correctGamma:
        inputs = srgb_to_linear(inputs)
    ...
    def transform(image):
        ...
        # Use BICUBIC for better quality resizing (closer to PIL's LANCZOS)
        # This provides better interpolation than AREA method
        r = tf.image.resize_images(r, [a.scale_size, a.scale_size], method=tf.image.ResizeMethod.BICUBIC)
        return r
